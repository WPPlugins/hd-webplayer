...
/* User Function
******************************************************************/
function webplayer_plugin_shortcode( $atts ) {
	global $wpdb;
	if(!$atts['id']) $atts['id'] = 1;;
	$player = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."webplayer WHERE id=".$atts['id']);
	
 	$siteurl = get_option('siteurl');
	$src     = $siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/player.swf';
	
	$flashvars = 'baseW='.$siteurl.'&id='.$player->id;

	$embed  = '';
	$html5  = '';

	if($player->videoid) {
		$results = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."webplayer_videos WHERE id=".$player->videoid);
	} else if ($player->playlistid) {
		$results = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."webplayer_videos WHERE playlistid=".$player->playlistid." LIMIT 1");
	}
...
add_shortcode('webplayer', 'webplayer_plugin_shortcode');
