...
class Webplayer_Table extends WP_List_Table {
    ...
    function column_default($item, $column_name){
		switch($column_name) {
			case 'actions' :
				if($item->id == 1) {
					return '<div style="margin-top:9px;"><a class="button-secondary" href="?page=webplayer&opt=edit&id='.$item->id.'" title="Edit">Edit</a></div>';
				} else {
					return '<div style="margin-top:9px;"><a class="button-secondary" href="?page=webplayer&opt=edit&id='.$item->id.'" title="Edit">Edit</a>&nbsp;&nbsp;&nbsp;<a class="button-secondary" href="?page=webplayer&opt=delete&id='.$item->id.'" title="Delete">Delete</a></div>';
				}
				break;
			case 'shortcode' :
				return '<div style="margin-top:4px;">[webplayer id='.$item->id.']</div>';
				break;
			case 'playlistid' :
    ...
    function process_bulk_action() {
		if( 'delete'===$this->current_action() ) {			
			...
			echo '<script>window.location="?page=webplayer";</script>';
		}
    }
...
<br />
<?php
	_e( "HD Webplayer is the Fastest Growing Online Video Platform for your Websites. For More visit <a href='http://hdwebplayer.com'>HD Webplayer</a>." );
	$table = new Webplayer_Table();
...
<br />
<form id="webplayer-filter" method="get" style="width:99%;">
